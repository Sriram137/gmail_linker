...
InboxSDK.load('1', 'sdk_mail_linker_80b609fbc9').then(function(sdk){
	...
	sdk.Conversations.registerMessageViewHandler(function(messageView) {
		messageView.addToolbarButton({
			section: 'MORE',
			title: 'Copy Email PermaLink',
			onClick: function () {

				messageView.getMessageIDAsync().then(function (messageId) {
					// When it's clicked, send a message to page.js
					barMessage = sdk.ButterBar.showSaving({text: "Loading...", confirmationText: "Message link copied to clipboard!"});
					window.postMessage({type: 'FETCH_ATTACHMENT', id: messageId}, "*");
				});
			},
			orderHint: 0
		});
